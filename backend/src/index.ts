...
function classifyTriggerSource(message: SimpleMessage | undefined): string {
  if (!message) {
    return 'unknown';
  }
...
function parseAuditJson(content: string): ParsedTurn[] {
  const parsed = JSON.parse(content) as unknown;
  if (!Array.isArray(parsed)) {
    throw new Error('Top-level JSON value must be an array.');
  }

  const events = parsed as AuditEvent[];
